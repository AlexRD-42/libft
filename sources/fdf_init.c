...
/*   Updated: 2025/06/22 11:54:54 by adeimlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline
float	fdf_lerp(const uint32_t min, const uint32_t max, const int32_t val)
{
...
void	fdf_create_vector(t_vars *vars)
{
	...
	while (i < vars->length)
	{
		...
		vars->vec[i].z = fdf_lerp(vars->min, vars->max, vars->fdf[i].height);
		i++;
