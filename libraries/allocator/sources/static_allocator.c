...
/*   Updated: 2025/12/17 19:00:21 by adeimlin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_free(void *ptr)
{
	...
	bytes = (uintptr_t)ptr - (uintptr_t)data;
	blocks = bytes / BLOCK_SIZE;
	if (ptr == NULL || bytes >= MEMORY_SIZE || blocks * BLOCK_SIZE != bytes)
		return ;
	end = ft_bitfind(metadata_end, blocks, MEMORY_SIZE / BLOCK_SIZE, 1);
	if (end > MEMORY_SIZE / BLOCK_SIZE)
		return ;
	if (ft_bitfind(metadata, blocks, end + 1, 0) > MEMORY_SIZE / BLOCK_SIZE)
		return ;
	ft_bitset(metadata, blocks, end + 1, 0);
